...
$version = "0.0.0-smoke"
$builds = @(@{ Executable = "bgr.exe"; Package = "./cmd/bgr" }, @{ Executable = "better-git-review.exe"; Package = "./cmd/better-git-review" })
$arch = switch ($env:PROCESSOR_ARCHITECTURE) {
  "AMD64" { "amd64" }
  "ARM64" { "arm64" }
  default { throw "unsupported smoke architecture: $env:PROCESSOR_ARCHITECTURE" }
}
$work = Join-Path ([System.IO.Path]::GetTempPath()) "bgr-install-smoke-$PID"
...
function Invoke-Installer([string]$Dist, [string]$Bin) {
  $env:BGR_INSTALL_BASE_URL = $Dist
  $env:BGR_INSTALL_VERSION = $version
  $env:BGR_INSTALL_DIR = $Bin
  $env:BGR_INSTALL_SKIP_PATH = "1"
...
try {
  ...
  try {
    foreach ($item in $builds) {
      & go build -buildvcs=false -ldflags "-s -w -X github.com/janiorvalle/better-git-review/internal/document.Version=$version" -o (Join-Path $build $item.Executable) $item.Package
      Assert ($LASTEXITCODE -eq 0) "go build $($item.Package) failed"
    }
  } finally {
    Pop-Location
  }
  $archiveName = "better-git-review_${version}_windows_${arch}.zip"
